...
func (conn *Connection) buildConnectRequest() []byte {
    // Target: 166.111.4.100:80 (same as SOCKS5 example)
    // Format: VN(1) + CD(1) + DSTPORT(2) + DSTIP(4) + USERID(variable) + NULL(1)

    req := []byte{
        0x04,       // VN: SOCKS version 4
        0x01,       // CD: CONNECT command
        0x00, 0x50, // DSTPORT: 80 (big-endian)
        0xA6, 0x6F, 0x04, 0x64, // DSTIP: 166.111.4.100
        0x00, // NULL terminator for USERID (empty user ID)
    }
...
func (conn *Connection) buildConnectRequestSocks4a(domain string) []byte {
    ...
    req := []byte{
        ...
        0x01,       // CD: CONNECT command
        0x00, 0x50, // DSTPORT: 80 (big-endian)
        0x00, 0x00, 0x00, 0x01, // DSTIP: 0.0.0.1 (signals SOCKS4a)
        0x00, // NULL terminator for USERID (empty user ID)
    }
...
func (conn *Connection) PerformConnectionRequest() error {
    ...
    if conn.config.UseSocks4a {
        req = conn.buildConnectRequestSocks4a("example.com")
